				...
				<view class="price detail-list-item priceFavor"
					...
					<view class="detail-list-item"
                        ...
                        <view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.huodongbaomingXingqu}}</view>
                    </view>
					<view class="detail-list-item"
                          :style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'>
                        <view class="lable"
							...
							意向岗位：</view>
                        <view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.huodongbaomingYixiang}}</view>
                    </view>
					<view class="detail-list-item"
						  :style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'>
						<view class="lable"
							...
							审核结果：</view>
						<view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.huodongbaomingYesnoValue}}</view>
					</view>
			</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('huodongbaomingState')
			if (cleanType) {
				uni.removeStorageSync('huodongbaomingState')
				this.mescroll.num = 1
		...
		methods: {
			...
			async init() {
				...
				// 主图
				this.swiperList = this.detail.huodongbaomingPhoto ? this.detail.huodongbaomingPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('huodongbaoming', {
					page: mescroll.num,
					limit: mescroll.size,
					huodongbaomingId: this.id
				});
				// 如果是第一页数据置空
